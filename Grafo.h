/*/ =================================================================
                    ...
                    Descrição:  Perunta ao usuário um diretorio e cria o Grafo
...
#define ORDENADO    <
//#define ORDENADO  !=
#define INFINITO  100000000     // Valor para auxiliar programação
...
bool inserir_aresta( Grafo &grafo, std::string v1, std::string v2, std::string aresta_rotulo, double aresta_custo)
{
    ...
    if ( v1 == v2 )
    {
        std::cout << std::endl << "*** Erro na insercao de aresta : grafo simples nao aceita lacos";
        return false;
    }
    procura_vertice( grafo, v1, vaux1 );
    for( auto ita : vaux1->arestas)
        if( ita.v->rotulo == v2 )
        {
